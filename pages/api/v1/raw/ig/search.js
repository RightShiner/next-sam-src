...
async function handler(req, res) {
  ...
  async function getSearch() {
    console.log('/api/v1/raw/ig/search');
    // let status = await UserRepo.reduceCredit(req.user.id, ReduceCredit.raw.search);   
    // if(status){
      return apiWrapper.get(`${baseUrl}/raw/ig/search`, req.query
      ).then(response => {
