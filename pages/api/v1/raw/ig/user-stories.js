...
async function handler(req, res) {
  switch (req.method) {
    case 'GET':
      return await getSotries();
    default:
      throw {status: Constants.errors.badrequest, message: Lang.communcation_errs.e009};
  }

  async function getSotries() {
    console.log('/api/v1/raw/ig/user-stories');
    let status = await UserRepo.reduceCredit(req.user.id, ReduceCredit.raw.user_story);   
    if(status){
      return apiWrapper.get(`${baseUrl}/raw/ig/user-stories`, req.query
      ).then(response => {
