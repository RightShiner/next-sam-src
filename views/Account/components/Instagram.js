...
import { modashService, accountService } from 'services';
...
export default function Instagram({
  ...
  const [isShowEngagementRateOnly, setIsShowEngagementRateOnly] = useState(
    false,
  );
  ...
  useEffect(() => {
    ...
    instagramFilters =
      instagramFilters !== undefined && instagramFilters !== null
        ? instagramFilters
        : null;

    let camps = JSON.parse(localStorage.getItem('instagramCamps'));
    camps = camps || [];

    setExceptCampaigns(camps);
    setFilters(instagramFilters);

    if (accounts.length === 0) {
      loadFromServer(0, sortOrder, sortDirection, instagramFilters);
    }
  }, [selected]);
  ...
  const loadFromServer = (
    ...
    setIsShowEngagementRateOnly(false);
    sortField ||= 'followers';
    sortDir ||= 'desc';
  ...
  const searchFromServer = (
    ...
    localStorage.setItem('instagramCamps', JSON.stringify(exceptCamps));
    ...
    filters.audience.location.map((value) => {
      audience.location.push(...value.val);
    });
    ...
    if (filters?.audience?.interests) {
      audience.interests = _.map(filters.audience.interests, (itm) => {
        return itm.id;
      });
    }

    audience.credibility = filters.audience.credibility || null;

    let influencer = {};
    influencer.accountTypes = filters.influencer.accountTypes || [];
    influencer.bio = filters.influencer.bio || '';
    ...
    influencer.hashtagEngagement = filters.influencer.hashtagEngagement || null;
    influencer.interests = filters.influencer.interests || [];
    influencer.keywords = filters.influencer.keywords || '';
    influencer.lastposted = filters.influencer.lastposted || null;
    influencer.reelsPlays = filters.influencer.reelsPlays || null;
    influencer.textTags = filters.influencer.textTags || null;

    influencer.topics = null;

    if (filters?.influencer?.topics) {
      influencer.topics = Array.prototype.map.call(
        filters.influencer.topics,
        (a) => a.label,
      );
    }
    ...
    if (filters?.influencer?.userlist) {
      ...
      if (
        ...
        influencer.userlist = Array.prototype.map.call(
          filters.influencer.userlist,
          (a) => (a.label?.startsWith('@') ? a.label : '@' + a.label),
        );
      }
    }
    ...
    if ((filterInfo?.influencer?.hashtagEngagement?.length ?? 0) === 0) {
      setIsShowEngagementRateOnly(false);
      return modashService
        .searchAccounts(
          Constants.snsInstagram,
        ...
        .then((response) => {
          ...
          const sorted = data.data.sort(
            (a, b) =>
              ...
              (sortField === 'followers'
                ? a.profile.followers - b.profile.followers
                : a.profile.engagements - b.profile.engagements),
          );
          setAccounts([...sorted]);
      ...
      setIsShowEngagementRateOnly(true);
      return accountService
        ...
        .then((response) => {
          ...
          // else setAccounts([...accounts, ...data.data]);
  ...
  const saveFromServer = async (searchFilters) => {
    ...
    if (searchFilters === null) {
      while (!breakSignal) {
        ...
        await modashService
          .getAccounts(Constants.snsInstagram, page, {})
          .then((response) => {
            if (response.status !== 'ok') {
              breakSignal = true;
              return;
            ...
            if (data.error) {
              ...
              data.data.forEach((itm) => {
                modashService
                  .getProfileReport(itm.userId, Constants.snsInstagram, 0)
                  .then((response) => {
      ...
      while (!breakSignal && page < 361) {
        console.log('page', page);

        await modashService
          .searchAccounts(Constants.snsInstagram, page, {}, searchFilters)
          .then(async (response) => {
            if (response.status !== 'ok') {
                ...
                await modashService
                  ...
                  .then((response) => {
                    if (response.data?.profile) {
                      let hashtagengage = [];
                      _.map(response.data?.popularPosts, (itm) => {
                        let likes = itm.likes ?? 0;
                        let comments = itm.comments ?? 0;
                        let followers = response.data?.profile?.followers ?? 0;
                      ...
                      csvData =
                        csvData +
                        [
                          response.data?.profile?.username, // IG Handle
                          response.data?.country, // Country
                          response.data?.profile?.fullname, // Fullname
                          response.data?.profile?.url ?? '', // URL
                          response.data?.profile?.followers, // #Followers/IG
                          (
                            response.data?.profile?.engagementRate * 100
                          ).toFixed(2), // %ER
                          (response.data?.audience?.genders[0]?.code === 'MALE'
                          ...
                          (
                            response.data?.audience?.gendersPerAge[5]?.male *
                            100
                          ).toFixed(2), // 65- Male
                          (
                            response.data?.audience?.gendersPerAge[0]?.female *
                            100
                          ).toFixed(2), // 13-17 Female
                          (
                            response.data?.audience?.gendersPerAge[1]?.female *
                            100
                          ).toFixed(2), // 18-24 Female
                          (
                            response.data?.audience?.gendersPerAge[2]?.female *
                            100
                          ).toFixed(2), // 25-34 Female
                          (
                            response.data?.audience?.gendersPerAge[3]?.female *
                            100
                          ).toFixed(2), // 35-44 Female
                          (
                            response.data?.audience?.gendersPerAge[4]?.female *
                            100
                          ).toFixed(2), // 45-64 Female
                          (
                            response.data?.audience?.gendersPerAge[5]?.female *
                            100
                          ).toFixed(2), // 65- Female
                          '"' +
                            ...
                            '=' +
                            (
                              (response.data?.audience?.geoCountries[0]
                                ?.weight ?? 0) * 100
                            ).toFixed(2) +
                            '"', // TOP 1 country
                          '"' +
                            ...
                            '=' +
                            (
                              (response.data?.audience?.geoCountries[1]
                                ?.weight ?? 0) * 100
                            ).toFixed(2) +
                            '"', // TOP 2 country
                          '"' +
                            ...
                            '=' +
                            (
                              (response.data?.audience?.geoCountries[2]
                                ?.weight ?? 0) * 100
                            ).toFixed(2) +
                            '"', // TOP 3 country
                          '"' +
                            (hashtagengage[0]?.tag ?? '') +
                            '=' +
                            (hashtagengage[0]?.weight ?? 0).toFixed(2) +
                            '"', // %Hashtg EG1
                          '"' +
                            (hashtagengage[1]?.tag ?? '') +
                            '=' +
                            (hashtagengage[1]?.weight ?? 0).toFixed(2) +
                            '"', // %Hashtg EG2
                          '"' +
                            (hashtagengage[2]?.tag ?? '') +
                            '=' +
                            (hashtagengage[2]?.weight ?? 0).toFixed(2) +
                            '"', // %Hashtg EG3
                          '"' +
                            (hashtagengage[3]?.tag ?? '') +
                            '=' +
                            (hashtagengage[3]?.weight ?? 0).toFixed(2) +
                            '"', // %Hashtg EG4
                          '"' +
                            (hashtagengage[4]?.tag ?? '') +
                            '=' +
                            (hashtagengage[4]?.weight ?? 0).toFixed(2) +
                            '"', // %Hashtg EG5
                          '"' +
                            (response.data?.audience?.interests[0]?.name ??
                              '') +
                            '=' +
                            (
                              (response.data?.audience?.interests[0]?.weight ??
                                0) * 100
                            ).toFixed(2) +
                            '"', // %Top1_Interest
                          '"' +
                            (response.data?.audience?.interests[1]?.name ??
                              '') +
                            '=' +
                            (
                              (response.data?.audience?.interests[1]?.weight ??
                                0) * 100
                            ).toFixed(2) +
                            '"', // %Top2_Interest
                          '"' +
                            (response.data?.audience?.interests[2]?.name ??
                              '') +
                            '=' +
                            (
                              (response.data?.audience?.interests[2]?.weight ??
                                0) * 100
                            ).toFixed(2) +
                            '"', // %Top3_Interest
                          '"' +
                            (response.data?.audience?.interests[3]?.name ??
                              '') +
                            '=' +
                            (
                              (response.data?.audience?.interests[3]?.weight ??
                                0) * 100
                            ).toFixed(2) +
                            '"', // %Top4_Interest
                          '"' +
                            (response.data?.audience?.interests[4]?.name ??
                              '') +
                            '=' +
                            (
                              (response.data?.audience?.interests[4]?.weight ??
                                0) * 100
                            ).toFixed(2) +
                            '"', // %Top5_Interest
                          '"' +
                            (response.data?.audience?.brandAffinity[0]?.name ??
                              '') +
                            '=' +
                            (
                              (response.data?.audience?.brandAffinity[0]
                                ?.weight ?? 0) * 100
                            ).toFixed(2) +
                            '"', // %Brand1
                          '"' +
                            (response.data?.audience?.brandAffinity[1]?.name ??
                              '') +
                            '=' +
                            (
                              (response.data?.audience?.brandAffinity[1]
                                ?.weight ?? 0) * 100
                            ).toFixed(2) +
                            '"', // %Brand2
                          '"' +
                            (response.data?.audience?.brandAffinity[2]?.name ??
                              '') +
                            '=' +
                            (
                              (response.data?.audience?.brandAffinity[2]
                                ?.weight ?? 0) * 100
                            ).toFixed(2) +
                            '"', // %Brand3
                          '"' +
                            (response.data?.audience?.brandAffinity[3]?.name ??
                              '') +
                            '=' +
                            (
                              (response.data?.audience?.brandAffinity[3]
                                ?.weight ?? 0) * 100
                            ).toFixed(2) +
                            '"', // %Brand4
                          '"' +
                            (response.data?.audience?.brandAffinity[4]?.name ??
                              '') +
                            '=' +
                            (
                              (response.data?.audience?.brandAffinity[4]
                                ?.weight ?? 0) * 100
                            ).toFixed(2) +
                            '"', // %Brand5
                          (
                            (response.data?.audience?.credibility ?? 0) * 100
                          ).toFixed(2), // %Follower Credibility
                          response.data?.avgLikes, // Avg likes
                          response.data?.avgComments, // Avg comments
                          (
                            (response.data?.audienceLikers.nonFollowerLikes ??
                              0) * 100
                          ).toFixed(2) + '%', // Non-follower likes
                          (
                            (response.data?.audienceLikers.credibility ?? 0) *
                            100
                          ).toFixed(2) + '%', // Audience likers credibility
        ...
        {directs.length > 0 && (
          ...
          >
            ...
            <Box className="research-content-header research-content-account-grid">
              ...
              <div>{Keyword[locale].caption.followers}</div>
              <div>{Keyword[locale].caption.engagement}</div>
              ...
              (itm) =>
                isShowable(itm.campaigns) && (
                  <SearchItem
                    ...
                    isShowEngagementRateOnly={isShowEngagementRateOnly}
                  />
                ),
            )}
          </Box>
          ...
          {_.map(accounts, (itm, index) => (
            <>
              {index % 15 === 0 && index / 15 > 0 && (
                <Box
                  className="research-content-header research-content-account-grid"
                  key={index / 15}
                >
                  <div></div>
                  <div></div>
