...
export default function Tiktok({
  ...
  useEffect(() => {
    ...
    tiktokFilters =
      tiktokFilters !== undefined && tiktokFilters !== null
        ? tiktokFilters
        : null;

    let camps = JSON.parse(localStorage.getItem('tiktokCamps'));
    camps = camps !== undefined && camps !== null ? camps : [];

    setExceptCampaigns(camps);

    if (accounts.length === 0) {
      loadFromServer(0, sortOrder, sortDirection, tiktokFilters);
    }
  }, [selected]);
  ...
  const loadFromServer = (
    ...
    if (searchFilters !== null) {
      searchFromServer(pageNum, sortField, sortDir, searchFilters);
      return;
    }
  ...
  const searchFromServer = (
    ...
    localStorage.setItem('tiktokCamps', JSON.stringify(exceptCamps));
  ...
  const saveFromServer = async (searchFilters) => {
    ...
    if (searchFilters === null) {
      while (!breakSignal) {
        ...
        await modashService
          .getAccounts(Constants.snsTiktok, page, {})
          .then((response) => {
            if (response.status !== 'ok') {
            ...
            if (data.error) {
              ...
              data.data.forEach((itm) => {
                modashService
                  .getProfileReport(itm.userId, Constants.snsTiktok, 0)
                  .then((response) => {
                    console.log('response.data', response.data);
      ...
      while (!breakSignal && page < 334) {
        console.log('page', page);

        await modashService
          .searchAccounts(Constants.snsTiktok, page, {}, searchFilters)
          .then(async (response) => {
            if (response.status !== 'ok') {
              console.log('response', response);
              page--;
                    ...
                    if (response.data?.profile) {
                      csvData =
                        ...
                        [
                          ...
                          response.data?.audience?.geoCountries[0]?.name ?? '', // TOP 1 country
                          response.data?.audience?.geoCountries[1]?.name ?? '', // TOP 2 country
                          response.data?.audience?.geoCountries[2]?.name ?? '', // TOP 3 country
