...
const Edit = ({ userInfo, cmpId, cmpName, cmpSns, cmpGenre, cmpList, cmpMembers }) => {
  const router = useRouter();
  const [title, setTitle] = useState(cmpName);
  const [sns, setSNS] = useState(cmpSns);
  const [genre, setGenre] = useState(cmpGenre);
  const [list, setList] = useState(cmpList);
  ...
  const campaignEdit = () => {
    ...
    return campaignService
      .updateCampaign(cmpId, title, sns, genre, list)
      .then((ret) => {
        if (ret.status === 'ok') {
        ...
        >
          ...
          <Box sx={{ textAlign: 'center' }}>
            <Button
              className="active"
              variant={'outlined'}
              onClick={(e) => campaignEdit()}
            >
              {Lang[router.locale].btn.save}
            </Button>
          </Box>
        </Paper>
      </Container>
