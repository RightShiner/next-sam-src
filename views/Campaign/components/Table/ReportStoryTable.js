...
const ReportStoryRow = ({ row, classes }) => {
  ...
  return (
    <>
      <TableRow>
        ...
        <TableCell className={classes.feedtableCell}>
          {formatterInt.format(row.inp)}
        </TableCell>
        <TableCell className={classes.feedtableCell}>
          {formatter.format(
            !row.inp || !row.followers ? 0 : (row.inp / row.followers) * 100,
          )}
        </TableCell>
        <TableCell className={classes.feedtableCell}>
        ...
        <TableCell className={classes.feedtableCell}>
          {formatter.format(
            !row.click || !row.followers
              ? 0
              : (row.click / row.followers) * 100,
          )}
        </TableCell>
        <TableCell className={classes.feedtableCell}>
          {formatterInt.format(row.stamp)}
        </TableCell>
        <TableCell className={classes.feedtableCell}>
          {formatter.format(
            !row.stamp || !row.followers
              ? 0
              : (row.stamp / row.followers) * 100,
          )}
        </TableCell>
        <TableCell className={classes.feedtableCell}>
...
export default function ReportStoryTable({ getDatas, classes, ...rest }) {
      ...
      >
        <TableContainer style={{ padding: 10 }}>
          ...
          >
            ...
            <TableBody>
              {data.map((row, index) => (
                <ReportStoryRow key={index} row={row} classes={classes} />
              ))}
            </TableBody>
          </Table>
        </TableContainer>
