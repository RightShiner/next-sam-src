...
const lang = {
  jp: {
    title: '登録者データ',
    notableSubscribers: '有名人率',
    notableSubscribersInfo:
      '登録者数1000人以上のアカウントが登録している率です。',
    languages: '言語',
    languagesInfo: '登録者の使っている言語。',
    audienceLookalikes: '類似アカウント',
    audienceLookalikesInfo:
      'キーワードトピックから類似アカウントを表示しています。',
    locationByCountry: '国',
    locationByCountryInfo: '登録者がどこの国にいるのか。',
    genderSplit: '男女比',
    genderSplitInfo: '登録者の男女比',
    male: '男',
    female: '女',
    ageAndGenderSplit: '年代別男女比',
    ageAndGenderSplitInfo: '登録者の年代別男女比率。',
  },
  en: {
    title: 'Audience data by subscribers',
    notableSubscribers: 'Notable subscribers',
    notableSubscribersInfo:
      'Top subscribers of this influencer with more than 1,000 subscribers',
    languages: 'Languages',
    languagesInfo:
...
const AudienceDataYoutube = ({ data, open }) => {
    ...
    Array.from(
      {
        length:
          !data || !data.audienceLookalikes
            ? 0
            : data.audienceLookalikes.length > 3
            ? 3
  ...
  useEffect(() => {
    if (!open || !data || !data.audienceLookalikes) return;
    ...
    async function fetchFromServer() {
      ...
      for (let idx = 0; idx < cnt; idx++) {
        ...
        // console.log(`Audience${idx} done`);
        avatars[idx] = `data:${result.type};base64,${result.blob}`;
      }
      setAvatars([...avatars]);
    }
      ...
      <Box className="wrapper-grid" sx={{ gridTemplateColumns: '1fr 1fr' }}>
        ...
        >
          ...
          <Box sx={{ display: 'flex', justifyContent: 'center' }}>
            <span>{lang[locale].notableSubscribers}</span>
            <RoundInfo
              sx={{ marginLeft: '.5rem' }}
              caption={lang[locale].notableSubscribersInfo}
            />
          </Box>
        </Box>
      </Box>
