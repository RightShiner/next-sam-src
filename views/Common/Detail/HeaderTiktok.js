...
const lang = {
  ...
  en: {
    ...
    followers: 'FOLLOWERS',
    followersInfo: 'How many total followers the influencer has on Tiktok.',
    ...
    async function fetchFromServer() {
      ...
      try {
        result = await imageService
          .getImageURL(encodeURIComponent(data.picture))
          .then(async (res) => {
      ...
      if (!result) return;
      //let dataUrl = URL.createObjectURL(result);
      //data:image/png;base64,
      setAvatarSrc(`data:${result.type};base64,${result.blob}`);
    }
