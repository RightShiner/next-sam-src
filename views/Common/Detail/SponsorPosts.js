...
const lang = {
  jp: {
    sponsoredPosts: 'PR投稿',
    sponsoredPostsInfo:
      'インフルエンサーが過去に投稿した中で、タイアップラベルのついている投稿。',
    dateFormat: 'YYYY年M月D日',
  },
  en: {
    sponsoredPosts: 'Sponsored Posts',
    sponsoredPostsInfo:
      'Posts with a tie-up label that influencers have posted in the past.',
...
const SponsorPosts = ({ open, data }) => {
  ...
  useEffect(() => {
    ...
    _.map(data, (itm, idx) => {
      getDataUri(itm.thumbnail, updateThumbs, idx);
    });
  }, [data, open]);
  const updateThumbs = (dataUrl, idx) => {
    thumbs[idx] = dataUrl;
  ...
  return (
    <Box className={classes.sponsorposts}>
      ...
      >
        <span>{lang[locale].sponsoredPosts}</span>
        <RoundInfo
          sx={{ marginLeft: '.5rem' }}
          caption={lang[locale].sponsoredPostsInfo}
        />
      </Box>
      ...
      <Box className="wrapper-grid" sx={{ gridTemplateColumns: '1fr 1fr 1fr' }}>
        {_.map(data, (itm, idx) => (
          <Box
            ...
            sx={{ width: '200px' }}
          >
            <Box className="subtitle1 mgl5 mgt5">{`${moment(itm.created).format(
              lang[locale].dateFormat,
            )}`}</Box>
