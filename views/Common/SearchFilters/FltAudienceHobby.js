...
export default function FltAudienceInterests({
  ...
  useEffect(() => {
    if (clearFlag === true || clearFlag === false) setInterests([], true);
  }, [clearFlag]);
  ...
  const setInterests = (value, forceClear = false) => {
    ...
    //setValues('interests', newFilterValue);
    if (!forceClear)
      setValues(
        'interests',
        _.map(newFilterValue, (itm) => {
          return itm.origin;
        }),
      );
  };
  ...
  const changeWeightValues = (idx, value) => {
    ...
    //setValues('interests', filterValue);
    setValues(
      'interests',
      _.map(filterValue, (itm) => {
        return itm.id;
      }),
    );
  };
