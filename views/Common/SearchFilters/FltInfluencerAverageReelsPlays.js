...
export default function FltInfluencerAverageReelsPlays({
  clearFlag,
  tip,
  caption,
  icon,
  ...
  useEffect(() => {
    if (clearFlag === true || clearFlag === false) {
      ...
      setItemValue(items);
      setAvgReelsPlays(items, true);
    }
  }, [clearFlag]);

  const setAvgReelsPlays = ({ from, to }, forceClear = false) => {
    setItemValue({ from, to });

    if (forceClear) return;
    if (from === '' && to === '') {
      setValues('reelsPlays', {});
    } else {
      setValues('reelsPlays', {
        min: from === '' ? null : from,
        max: to === '' || to === '3000000+' ? null : to,
      });
    }
  };
    ...
    <Box className="flex-sub-wrapper">
      <Box className="search-item-wrapper">
        ...
        {(itemValue.from !== '' || itemValue.to !== '') && (
          <span
            className="clear"
            onClick={(e) => setAvgReelsPlays({ from: '', to: '' })}
          >
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
        <Select
          ...
          onChange={(e) =>
            setAvgReelsPlays({ ...itemValue, from: e.target.value })
          }
          className={clsx(
        ...
        <Select
          ...
          value={itemValue.to}
          onChange={(e) =>
            setAvgReelsPlays({ ...itemValue, to: e.target.value })
          }
