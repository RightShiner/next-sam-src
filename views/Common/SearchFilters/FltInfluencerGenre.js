...
export default function FltInfluencerGenre({
  clearFlag,
  tip,
  icon,
  values,
  ...
  useEffect(() => {
    if (clearFlag === true || clearFlag === false) setGenre('', true);
  }, [clearFlag]);

  const setGenre = (val, forceClear = false) => {
    setItemValue(val);

    if (!forceClear) setValues('genre', val === '' ? null : val);
  };
    ...
    <Box className="flex-sub-wrapper">
      ...
      >
        {_.map(values, (itm, itmIdx) => (
          <MenuItem key={itmIdx} value={itm} sx={{ fontSize: '14px' }}>
            {itm}
          </MenuItem>
        ))}
      </Select>
