...
export default function FltInfluencerPost({
  clearFlag,
  tip,
  icon,
  values,
    ...
    if (clearFlag === true || clearFlag === false) {
      let items =
        JSON.parse(localStorage.getItem(`${type}Filters`))?.influencer
          ?.lastposted || '';
      setItemValue(items);
      changeItemValue(items, true);
    }
  }, [clearFlag]);

  const changeItemValue = (val, forceClear = false) => {
    setItemValue(val);
    if (!forceClear) setValues('lastposted', val === '' ? null : val);
  };
