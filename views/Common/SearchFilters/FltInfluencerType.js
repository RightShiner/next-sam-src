...
export default function FltInfluencerType({
  clearFlag,
  tip,
  icon,
  values,
  ...
  useEffect(() => {
    if (clearFlag === true || clearFlag === false) setEngagements('', true);
  }, [clearFlag]);

  const setEngagements = (val, forceClear = false) => {
    setItemValue(val);

    if (!forceClear) setValues('type', val === '' ? null : val);
  };
  ...
  return (
    <Box className="flex-sub-wrapper">
      ...
      <Select
        ...
        {...rest}
      >
        {_.map(values, (itm, itmIdx) => (
          <MenuItem key={itmIdx} value={itm.value} sx={{ fontSize: '14px' }}>
            {itm.text}
          </MenuItem>
        ))}
      </Select>
