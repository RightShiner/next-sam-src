...
const initInfluencer = {
  ...
  tags: [],
  username: [],
  youtube: { click: null, cv: null, eg: null, roas: null },
};
...
export default function YoutubeFilter({ loadFromServer, ...rest }) {
  ...
  const setFeedFilter = (field, value) => {
    influencerFilter.youtube[field] = value;
      ...
      <Box className="search-box">
        <Box sx={{ display: 'flex', flexShrink: 0 }}>
          <Box>
            <FltInsightTextField
              clearFlag={clearFlag}
              tip={lang[locale].click}
              icon={false}
              phstr={lang[locale].click}
