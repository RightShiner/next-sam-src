...
import { YoutubeFilter } from './Filters';
...
const YouTube = ({ user, tags, selected }) => {
  ...
  const loadFromServer = async (curFilters, curPage) => {
    ...
    return accountService
      .getInfluencers(user.id, Constant.snsYoutube, curFilters, curPage)
      .then((response) => {
  ...
  return (
    <Box sx={{ display: `${selected ? 'block' : 'none'}` }}>
      <YoutubeFilter loadFromServer={loadWithFilter} />
      <SearchContent
